...
SIZE_T __cdecl QueryWorkingSetEx64(HANDLE hProcess, PPSAPI_WORKING_SET_EX_INFORMATION64 lpBuffer, SIZE_T dwLength)
{
	static DWORD64 ntqvmg = 0;
	if (0 == ntqvmg)
	{
		ntqvmg = GetProcAddress64(getNTDLL64(), "NtQueryVirtualMemory");
		if (0 == ntqvmg)
			return 0;
	}
	DWORD64 ret = 0;
	DWORD64 status = X64Call(ntqvmg, 6, (DWORD64)hProcess, (DWORD64)0, (DWORD64)4, (DWORD64)lpBuffer, (DWORD64)dwLength, (DWORD64)&ret);
...
{
	static DWORD64 nwvm = 0;
	if (0 == nwvm)
	{
		nwvm = GetProcAddress64(getNTDLL64(), "NtWriteVirtualMemory");
		if (0 == nwvm)
			return 0;
	}
	DWORD64 numOfBytes = lpNumberOfBytesWritten ? *lpNumberOfBytesWritten : 0;
	DWORD64 ret = X64Call(nwvm, 5, (DWORD64)hProcess, lpBaseAddress, (DWORD64)lpBuffer, (DWORD64)nSize, (DWORD64)&numOfBytes);
	if (STATUS_SUCCESS != ret)
	{
		SetLastErrorFromX64Call(ret);
		return FALSE;
...
BOOLEAN __cdecl NtQueryInformationThread64(HANDLE ThreadHandle, THREADINFOCLASS ThreadInformationClass, DWORD64 ThreadInformation, ULONG nSize, PULONG lpNumberOfBytesWritten)
{
	static DWORD64 nqit = 0;
	if (0 == nqit)
	{
		nqit = GetProcAddress64(getNTDLL64(), "NtQueryInformationThread");
		if (0 == nqit)
			return FALSE;
	}
	DWORD64 numOfBytes = lpNumberOfBytesWritten ? *lpNumberOfBytesWritten : 0;
	DWORD64 ret = X64Call(nqit, 5, (DWORD64)ThreadHandle, (DWORD64)ThreadInformationClass, (DWORD64)ThreadInformation, (DWORD64)nSize, (DWORD64)&numOfBytes);
	if (STATUS_SUCCESS != ret)
	{
		SetLastErrorFromX64Call(ret);
		return FALSE;
